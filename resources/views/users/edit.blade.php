...
<div class="form-container">
    ...
    <form method="POST" action="{{ route('users.update', $user->id) }}" class="user-form">
        ...
        <div class="form-group">
            ...
            <input type="email" name="email" value="{{ old('email', $user->email) }}" placeholder="exemple@domaine.com" required>
        </div>

        <!-- Téléphone -->
        <div class="form-group">
            <label><i class="fas fa-phone"></i> Téléphone</label>
            <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" placeholder="Numéro de téléphone" required>
        </div>

        <!-- Numéro de carte d'identité -->
        <div class="form-group">
            <label><i class="fas fa-id-card"></i> Numéro de carte d'identité</label>
            <input type="text" name="identity_card_number" value="{{ old('identity_card_number', $user->identity_card_number) }}" placeholder="Numéro de la carte d'identité">
        </div>

        <!-- Adresse -->
        <div class="form-group">
            <label><i class="fas fa-map-marker-alt"></i> Adresse</label>
            <input type="text" name="address" value="{{ old('address', $user->address) }}" placeholder="Adresse complète">
        </div>

        <!-- Personne à prévenir -->
        <div class="form-group">
            <label><i class="fas fa-user-friends"></i> Nom de la personne à prévenir</label>
            <input type="text" name="emergency_contact_name" value="{{ old('emergency_contact_name', $user->emergency_contact_name) }}" placeholder="Nom de la personne à prévenir">
        </div>

        <!-- Contact personne à prévenir -->
        <div class="form-group">
            <label><i class="fas fa-phone-alt"></i> Contact de la personne à prévenir</label>
            <input type="text" name="emergency_contact_phone" value="{{ old('emergency_contact_phone', $user->emergency_contact_phone) }}" placeholder="Téléphone de la personne à prévenir">
        </div>
